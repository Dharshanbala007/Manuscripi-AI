...
test.describe("Theme", () => {
  ...
  test.describe("reduced motion", () => {
    ...
    test("the silk renders one still frame", async ({ page }) => {
      await page.goto("/");
      const a = await silkFingerprint(page);
      await page.waitForTimeout(700);
      expect(a).toBeGreaterThan(0);
  ...
  test("the toggle switches to light, drops the silk, and persists across reloads", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("button", { name: "Switch to light theme" }).click();
    await expect(page.locator("html")).toHaveAttribute("data-theme", "light");
    await expect(page.getByTestId("silk-background")).toHaveCount(0);
